...
void exponentiate (gwhandle *gwdata, gwnum x, uint64_t power)
{
	...
	if (power <= 64) { simple_exponentiate (gwdata, x, power); return; }
...
//BUG	if (x1 == NULL || x3 == NULL || x5 == NULL || x7 == NULL) goto oom;
	...
	top3 = 4;
	if (power & (current_bit >> 1)) top3 += 2;
	if (power & (current_bit >> 2)) top3 += 1;
	...
	for (current_bit >>= 3; current_bit; current_bit >>= 1) {
		if (first_squaring_done)
			first_squaring_done = 0;
		else {
			gwstartnextfft (gwdata, current_bit != 1 || (power & current_bit));
			gwsquare (gwdata, x);
		}
		if ((power & current_bit) == 0) continue;
		// We have a window starting with a set bit, see if we want to apply x^1, x^3, x^5, or x^7
		if (power & (current_bit >> 2)) {
			gwstartnextfft (gwdata, 1);
			gwsquare (gwdata, x);
			gwsquare (gwdata, x);
			if (power & (current_bit >> 1)) {	// Apply x^7
				if (x7_is_really_x2) { gwfftfftmul (gwdata, x7, x5, x7); x7_is_really_x2 = 0; gwfft (gwdata, x7, x7); }
				gwstartnextfft (gwdata, current_bit != 4);
			...
			if (power & (current_bit >> 1)) {	// Apply x^3
				gwstartnextfft (gwdata, 1);
				gwsquare (gwdata, x);
