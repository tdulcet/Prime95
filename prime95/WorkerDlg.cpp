...
void CWorkerDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDV_MinMaxUInt(pDX, m_num_thread, 1, NUM_CPUS * CPU_HYPERTHREADS);
	...
	//}}AFX_DATA_MAP
	c_num_thread_text.EnableWindow (NUM_CPUS * CPU_HYPERTHREADS > 1);
	c_num_thread.EnableWindow (NUM_CPUS * CPU_HYPERTHREADS > 1);
	...
	c_numcpus_text.EnableWindow (NUM_CPUS * CPU_HYPERTHREADS > 1);
	c_numcpus.EnableWindow (NUM_CPUS * CPU_HYPERTHREADS > 1);
}
...
// Populate the thread number combo box
	...
	if (m_num_thread == 1) {
		thread_num = 1;
		c_threadnum.AddString ("Worker thread #1");
		c_threadnum.SetCurSel (0);
	} else {
		if (thread_num > (int) m_num_thread) thread_num = 0;
		c_threadnum.AddString ("All worker threads");
		for (i = 1; i <= (int) m_num_thread; i++) {
...
// Populate the work type combo box
	...
	c_work_pref.AddString ("10,000,000 digit numbers to test");
	if (!USE_V4)
...
void CWorkerDlg::OnEnKillfocusNumThread()
{
	...
	if (num_thread > NUM_CPUS * CPU_HYPERTHREADS)
		num_thread = NUM_CPUS * CPU_HYPERTHREADS;
