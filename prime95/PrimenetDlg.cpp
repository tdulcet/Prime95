...
PrimenetV4Dlg::PrimenetV4Dlg(CWnd* pParent /*=NULL*/)
	: CDialog(PrimenetV4Dlg::IDD, pParent)
{
	//{{AFX_DATA_INIT(PrimenetV4Dlg)
	m_primenet = FALSE;
	m_userid = _T("");
	m_userpwd = _T("");
	m_username = _T("");
...
void PrimenetV4Dlg::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(PrimenetDlg)
	DDX_Check(pDX, IDC_PRIMENET, m_primenet);
	...
	DDV_MaxChars(pDX, m_userpwd, 8);
	DDX_Text(pDX, IDC_USERNAME, m_username);
	DDV_MaxChars(pDX, m_username, 79);
	DDX_Text(pDX, IDC_COMPID, m_compid);
	DDV_MaxChars(pDX, m_compid, 20);
	DDX_Control(pDX, IDC_CONNECTION, c_connection);
	//}}AFX_DATA_MAP
...
BEGIN_MESSAGE_MAP(PrimenetV4Dlg, CDialog)
	//{{AFX_MSG_MAP(PrimenetV4Dlg)
	ON_BN_CLICKED(IDC_PRIMENET, OnPrimenet)
	ON_BN_CLICKED(IDC_CONNECTION, OnConnection)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()
...
/////////////////////////////////////////////////////////////////////////////
// PrimenetV4Dlg message handlers

void PrimenetV4Dlg::OnPrimenet() 
{
	UpdateData ();		// Get the values from the dialog box
}

void PrimenetV4Dlg::OnConnection() 
{
	PrimenetConnectionDlg dlg;
